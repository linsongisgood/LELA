...
class xlm_robertamodel(nn.Module):
    def __init__(self, config):
        ...
        self.out_proj1 = nn.Linear(config.hidden_size, 3)
        self.out_proj2 = nn.Linear(config.hidden_size, 3)
    ...
    def forward(self, input_ids ,attention_mask ,p1,
        ...
        loss4 = loss_fct(logits.view(-1, 3), labels.view(-1))
        loss5 = loss_fct(logits1.view(-1, 3), labels.view(-1))
        loss6 = loss_fct(logits2.view(-1, 3), labels.view(-1))

        logits = logits+logits1+logits2
        loss  = 0.7*(loss4+loss5+loss6)+0.3*(loss1+loss2+loss3)*tmp*tmp
        #print(0.4 * (loss1 + loss2 + loss3) * tmp * tmp, 0.6 * (loss4 + loss5 + loss6))
        return loss, logits
