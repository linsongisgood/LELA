...
class PLoraLayer:
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        ...
        if r > 0:
            ...
            self.lora_C.update(nn.ModuleDict({adapter_name: nn.Linear(r, self.out_features, bias=False)}))
...
class Linear(nn.Linear, PLoraLayer):
    ...
    def forward(self, x: torch.Tensor, p: Optional[torch.Tensor] = None, mim_list: Optional[list] = None):
        ...
        if self.disable_adapters:
            ...
            #
            #              (self.lora_dropout[self.active_adapter](p)))
            #         )
            #         * self.scaling[self.active_adapter]
            # )
            result_p = (
                (self.lora_B[self.active_adapter]
                 (self.lora_A[self.active_adapter](self.lora_dropout[self.active_adapter](x))) +
             self.lora_C[self.active_adapter](self.lora_P[self.active_adapter](
                            self.lora_dropout[self.active_adapter](p)))
                    )
                    * self.scaling[self.active_adapter]
            )
